...
def return_args_oracle_server_dlrm(
    private_ip_trainers, private_ip_oracle_cacher, batch_size, cache_size, stop_iter
):
    """
...
def run_large_scale():
    ...
    cache_size_kaggle = kaggle_size[2]
    cache_size_fgcnn = fgcnn_size[1]
    iterations = 500
    with_dlrm = True
    with_wdn = False
    with_dcn = False
    ...
    if with_dlrm:
        # ========Launching Bagpipe run 1========================================
        cache_size = cache_size_kaggle
        log_file_name = "run_dlrm_{}_num_machines_{}_run_2".format(
            len(private_ip_trainers), batch_size
        )
        run_args_trainers = return_args_trainers_dlrm(
            private_ip_trainers,
